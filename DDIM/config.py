...
data_paths = {
    ...
    'latent_feature':
    os.path.expanduser('/mnt/petrelfs/yangmengping/ICGAN/coco/COCO128_feats_selfsupervised_resnet50.hdf5'),
    'coco128':
    os.path.expanduser('/mnt/petrelfs/yangmengping/data/coco/train2017.lmdb'),
    'bedroom256':
...
class TrainConfig(BaseConfig):
    ...
    def make_dataset(self, path=None, **kwargs):
        if self.data_name == 'ffhqlmdb256':
            ...
            return Horse_lmdb(path=path or self.data_path,
                              image_size=self.img_size,
                              **kwargs)
        elif self.data_name == 'coco128':
            return COCO_lmdb(path=path or self.data_path,
                              image_size=self.img_size,
                              **kwargs)
            ...
            return CelebAlmdb(path=path or self.data_path,
                              ...
                              crop_d2c=True,
                              **kwargs)
        elif self.data_name == 'latent_feature':
            return LatentDataset(file_path=path,
                                flip=False,
                )
        else:
            raise NotImplementedError()
    ...
    def make_model_conf(self):
        ...
        elif self.model_name == ModelName.latentnet:
            self.model_type = ModelType.latentnet
            self.model_conf = MLPSkipNetConfig(
                    num_channels=self.style_ch,
                    skip_layers=self.net_latent_skip_layers,
                    num_hid_channels=self.net_latent_num_hid_channels,
                    num_layers=self.net_latent_layers,
