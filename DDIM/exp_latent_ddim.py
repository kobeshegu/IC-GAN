...
class LitModel(pl.LightningModule):
    def __init__(self, conf: TrainConfig):
        ...
        self.register_buffer(
            'x_T',
            torch.randn(conf.sample_size, conf.feature_size)
            )
        ...
        # load infered conds from local path
    ...
    def sample(self, N, device, T=None):
        if T is None:
            ...
            latent_sampler = self.conf._make_latent_diffusion_conf(T).make_sampler()

        noise = torch.randn(N, self.conf.feature_size, device=device)
        pred_noise = render_uncondition_feat(
            self.cond, 
            self.ema_model, 
            noise,
    ...
    def forward(self, noise=None, x_start=None, ema_model: bool = False):
        with amp.autocast(False):
            if ema_model:
                ...
                model = self.model
            gen = self.eval_latent_sampler.sample(model=model,
                                           noise=noise,
                                           x_start=x_start)
            return gen
    ...
    def setup(self, stage=None) -> None:
        ...
        self.train_data = self.conf.make_dataset(path='/mnt/petrelfs/yangmengping/ICGAN/coco/COCO128_feats_selfsupervised_resnet50.hdf5')
    ...
    def training_step(self, batch, batch_idx):
        ...
        losses = self.all_gather(losses).mean()
        # for key in ['loss', 'vae', 'latent', 'mmd', 'chamfer', 'arg_cnt']:
        #     if key in losses:
        #         losses[key] = self.all_gather(losses[key]).mean()

        if self.global_rank == 0:
            self.logger.experiment.add_scalar('loss', losses,
                                              self.num_samples)
        return {'loss': losses}
    ...
    def log_sample(self, x_start):
        """
        put tensors to the tensorboard
        """
        def do(model,
               postfix,
               use_xstart,
    ...
    def evaluate_scores(self):
        ...
        def l1(model, postfix):
            if self.conf.model_type == ModelType.latentnet and self.conf.train_mode == TrainMode.latent_diffusion:
                score = evaluate_l1(self.eval_latent_sampler,
                    model,
                    self.conf,
                    device=self.device,
                    val_data=self.val_data
        ...
        if self.conf.eval_every_samples > 0 and self.num_samples > 0 and is_time(
                ...
                self.conf.batch_size_effective):
            print(f'eval l1 @ {self.num_samples}')
            # lpips(self.model, '')
            # fid(self.model, '')
            l1(self.model,'')
        ...
        if self.conf.eval_ema_every_samples > 0 and self.num_samples > 0 and is_time(
                ...
                self.conf.batch_size_effective):
            print(f'eval l1 ema @ {self.num_samples}')
            #fid(self.ema_model, '_ema')
            l1(self.ema_model,'_ema')
            # it's too slow
